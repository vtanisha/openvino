...
namespace {
constexpr std::array<const char*, RegPrints::vec_cnt> vreg_names = {
    "v0",  "v1",  "v2",  "v3",  "v4",  "v5",  "v6",  "v7",  "v8",  "v9",  "v10", "v11", "v12", "v13", "v14", "v15",
    "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23", "v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31"};

constexpr std::array<const char*, RegPrints::reg_cnt> xreg_names = {
    "x0",  "x1",  "x2",  "x3",  "x4",  "x5",  "x6",  "x7",  "x8",  "x9",  "x10", "x11", "x12", "x13", "x14", "x15",
    "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27", "x28", "x29", "x30"};

constexpr std::array<const char*, RegPrints::reg_cnt> wreg_names = {
...
void RegPrints::print_reg_common(jit_generator_t& h, const REG_T& reg, const char* name, PrinterFunc printer) {
    preamble(h);
...
void RegPrints::print_vmm(jit_generator_t& h, const REG_T& vmm, const char* name) {
    auto printer = &ov::intel_cpu::debug_utils::print_vmm_prc<PRC_T, RegPrints::vec_len>;
    print_reg_common<PRC_T>(h, vmm, name, printer);
}

template <typename PRC_T, typename REG_T>
void RegPrints::print_reg(jit_generator_t& h, const REG_T& reg, const char* name) {
    auto printer = &ov::intel_cpu::debug_utils::print_reg_prc<PRC_T>;
    print_reg_common<PRC_T>(h, reg, name, printer);
}

void RegPrints::preamble(jit_generator_t& h) {
    save_reg(h);
    save_vmm(h);
}

void RegPrints::postamble(jit_generator_t& h) {
    restore_vmm(h);
    restore_reg(h);
}

void RegPrints::save_reg(jit_generator_t& h) {
...
void RegPrints::restore_reg(jit_generator_t& h) {
    size_t offset = 0;
    for (size_t idx = 0; idx + 1 < reg_cnt; idx += 2) {
        const auto current_offset = static_cast<int32_t>(offset);
...
void RegPrints::save_vmm(jit_generator_t& h) {
...
void RegPrints::restore_vmm(jit_generator_t& h) {
    size_t offset = 0;
    for (size_t idx = 0; idx < vec_cnt; idx += 2) {
        const auto current_offset = static_cast<int32_t>(offset);
...
template void RegPrints::print<float, VReg>(jit_generator_t& h, VReg reg, const char* name);
template void RegPrints::print<int, VReg>(jit_generator_t& h, VReg reg, const char* name);
template void RegPrints::print<float, XReg>(jit_generator_t& h, XReg reg, const char* name);
template void RegPrints::print<int, XReg>(jit_generator_t& h, XReg reg, const char* name);
template void RegPrints::print<float, WReg>(jit_generator_t& h, WReg reg, const char* name);
template void RegPrints::print<int, WReg>(jit_generator_t& h, WReg reg, const char* name);
